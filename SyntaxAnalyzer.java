...
updateSymbolTableGUI(symbolTableArea); // Update GUI
 ...
 private static void checkMainFunction(String code, JTextArea outputArea) {
    ...
    if (!hasProperMain) {
        ...
        if (hasIncorrectMain) {
            ...
            if (anyMainWord) {
                ...
                outputArea.append("Error: No main() function found. C++ programs require a main function.\n");
            }
        }
        return;
    }
    ...
    private static void checkSyntaxErrors(String line, int lineNumber, JTextArea outputArea) {
        ...
        Pattern strayPunctuation = Pattern.compile("(?<![\"'\\w\\s\\\\])([!#%^&*+\\-=\\[\\]|~])(?![\"'\\w\\s\\\\])");
    ...
    private static boolean checkIfElseStatements(String line, int lineNumber, JTextArea outputArea) {
        ...
        if (line.matches("^\\s*if\\s*\\((.*)\\)\\s*(\\{)?\\s*$")) {
            String condition = line.replaceAll("^\\s*if\\s*\\((.*)\\)\\s*(\\{)?\\s*$", "$1");
            ...
            foundIf = true;
        } else if (line.matches("^\\s*else\\s+if\\s*\\((.*)\\)\\s*(\\{)?\\s*$")) {
            String condition = line.replaceAll("^\\s*else\\s+if\\s*\\((.*)\\)\\s*(\\{)?\\s*$", "$1");
            checkConditionVariables(condition, lineNumber, outputArea);

            if (condition.trim().isEmpty()) {
                outputArea.append("Line " + lineNumber + ": Empty condition in else-if statement.\n");
            }
